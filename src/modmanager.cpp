...
ModManager::ModManager()
{
    ...
    QList<Mod*>* sectionList = nullptr;

    // The name of the current section being parsed
    QString sectionName;
    ...
    while (!in.atEnd()) {
        ...
        // Also mark the real mod (from the scanned folder list) as enabled
        // and remember which section it belongs to
        for (Mod *realMod : std::as_const(this->mods)) {
            if (realMod->directory.dirName() == line) {
                realMod->enabled = true;
                realMod->loadOrderSection = sectionName;
                break;
            }
        }
    }
...
bool ModManager::writeLoadOrder(const QList<Mod *> &mods)
{
    ...
    if (!loadOrderFile.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text)) {
        qWarning() << "Failed to write mod load order file:" << loadOrderFile.errorString();
        return false;
    }
    ...
    out << "; KeeperFX mod load order - generated by the launcher\n";

    // Write the three sections in order
    const QStringList sections { "after_base", "after_campaign", "after_map" };
    for (const QString &section : sections) {
        out << '[' << section << "]\n";

        // List every enabled mod that belongs to this section
        for (const Mod *mod : std::as_const(mods)) {
            if (mod->enabled && mod->loadOrderSection == section) {
                out << mod->directory.dirName() << '\n';
            }
        }
    }
